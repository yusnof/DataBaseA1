SELECT jsonb_build_object(
    ...
    'program', program , 
    'branch', branch, 
    'finshed', (SELECT jsonb_agg(jsonb_build_object('course', courseName, 'code', course , 'credits', credits, 'grade', grade)) 
    FROM FinishedCourses WHERE student='2222222222'),
    ...
    FROM 
    (SELECT c.name , r.course, r.status, w.position FROM Registrations r
    JOIN Courses c ON r.course = c.code 
    JOIN WaitingList w ON w.student = c.name
    WHERE r.student = '2222222222')AS x), '[]':: jsonb),
